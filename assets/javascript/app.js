...
//toggle for additive gifs, or for fresh ones each time a button is pressed
    ...
    $('#add').on("click", function() {
        ...
        $('#button_' + newIndex).data("name", newButton);
        console.log($(this).data("name"));
    });
    $('.gen-button').on("click", function () {
        // var array;
        queryURL = "https://api.giphy.com/v1/gifs/search?q=" + $(this).data("name") + "&api_key=s9kLWgDVn7rUQdMDmUSknpwBTvdwPrzT&limit=50";
        console.log($(this).data("name"));
        x = 10;
        $.ajax({
            url: queryURL,
            ...
            function addGifs(x) {
                ...
                overlay.append('<h5 class="card-title gif-name hide">' + array.data[x].title + '</h5>' + '<div class="fav-div"><i class="fas fa-star fav btn hide"></i></div>' +
                    '<button class="btn dl-btn hide"><i class="fa fa-download"></i> Download</button>');
                gifCard.append(overlay);
                $(".fav").on("click", function () {
                    console.log("yup");
                    $(".fav-div").empty();
                    $(".fav-div").html('<i class="fas fa-star fav btn hide"></i>');
